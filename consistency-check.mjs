...
for (const f of files) {
  const src = readFileSync(f, "utf8");
  const names = new Set();
  for (const m of src.matchAll(/export\s+(?:async\s+)?(?:function|const|class|type|interface|enum)\s+(\w+)/g)) names.add(m[1]);
  ...
  for (const m of src.matchAll(/import\s+(?:type\s+)?\{([^}]+)\}\s+from\s+["'](@\/[^"']+|\.[^"']+)["']/g)) {
    ...
    for (const part of m[1].split(",")) {
      ...
      if (name && !avail.has(name))
        problems.push(`MISSING EXPORT  ${f.replace(ROOT + "/", "")} imports "${name}" from ${m[2]}`);
    }
  }
}
...
for (const f of dictFiles) {
  ...
  for (const l of langs.filter((x) => x !== "en")) {
    for (const k of base) if (!perLang.get(l).has(k))
      problems.push(`UNTRANSLATED  ${f.replace(ROOT + "/", "")} [${l}] missing ${k}`);
    for (const k of perLang.get(l)) if (!base.has(k))
