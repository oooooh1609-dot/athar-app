...
const { normalise, tokenise, searchProjects } = await loadTs("../src/lib/search.ts", import.meta.url);
...
console.log("— ranking —");
const P = (o) => ({ name:"", notes:"", versions:[], annotations:[], createdAt: Date.now(), ...o });
const projects = [
  P({ id:"a", name:"Hdr stone", notes:"" }),
  P({ id:"b", name:"Unrelated", notes:"contains shdrq inside a word" }),
  P({ id:"c", name:"Boulder 4", machineReading:{ transliteration:"ḥḏr bn ʾlh", proposedReading:"", meaning:"", script:"Thamudic", references:[] } }),
];
...
is("arabic query finds arabic", searchProjects([P({id:"d",name:"نقش مُحَمَّد"})], "محمد").length, 1);
