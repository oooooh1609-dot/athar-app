...
is("openai key", resolveProvider({ OPENAI_API_KEY: "sk-x" }).url, "https://api.openai.com/v1/chat/completions");
...
is("openai over lovable", resolveProvider({ OPENAI_API_KEY: "a", LOVABLE_API_KEY: "b" }).name, "openai-compatible");
is("model override", resolveProvider({ OPENAI_API_KEY: "a", ATHAR_AI_MODEL: "gpt-4o-mini" }).model, "gpt-4o-mini");
is("bearer header", resolveProvider({ OPENAI_API_KEY: "sk-9" }).headers.Authorization, "Bearer sk-9");
...
JSON.parse(JSON.stringify(rb)); JSON.parse(JSON.stringify(cb));
console.log("PASS  both bodies serialise");

console.log("— error messages —");
is("401 names the key", describeFailure(401, "openai-compatible").includes("key"), true);
is("404 names the model", describeFailure(404, "openai-compatible").includes("ATHAR_AI_MODEL"), true);
