...
const near = (n, g, w, tol = 1e-6) => {
  const ok = Math.abs(g - w) <= tol;
  if (!ok) bad++;
  console.log(
    `${ok ? "PASS" : "FAIL"}  ${n}: ${typeof g === "number" ? g.toFixed(4) : g}${ok ? "" : ` (want ~${w})`}`,
  );
};
...
for (const yaw of [-180, -90, 0, 90, 179])
  for (const pitch of [-85, -40, 0, 40, 85])
    for (const px of [0, W / 2, W - 1])
      for (const py of [0, H / 2, H - 1]) {
