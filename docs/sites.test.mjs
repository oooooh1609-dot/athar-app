...
const a = at("a", 26.78, 37.95);
const b = at("b", 26.78027, 37.95);
const c = at("c", 26.78054, 37.95);
const far = at("far", 26.8, 37.95);
const nowhere = {
  id: "n",
  name: "n",
  notes: "",
...
is(
  "distant member chained in",
  sites[0].projects.some((p) => p.id === "c"),
  true,
);
is("far one is separate", sites[1].projects[0].id, "far");
is(
  "unlocated excluded",
  sites.flatMap((s) => s.projects).some((p) => p.id === "n"),
  false,
);
